    ...
    if (!guildId)
      ...
      if (!guildInCache)
        ...
        const ownerId = guildInCache.ownerId,
          ...
          ownerFlags = owner?.user.flags?.toArray();

        if (!owner)
          interaction.reply({content: "Owner not found", flags: ["Ephemeral"]});
        else if (!ownerFlags)
          interaction.reply({
            content: "Owner user flags not found",
            flags: ["Ephemeral"],
          });
        else {
          ...
          interaction
            .update({
              embeds: [
                {
                  author: {
                    name: `User flags of ${owner.displayName} (${ownerId})`,
                    icon_url: owner.displayAvatarURL(),
                  },
                  fields:
                    fields.length == 0
                      ? [{name: "No user flags", value: "\u200b"}]
                      : fields,
                },
              ],
              components: [
            ...
            .catch((error) => {
              ...
              console.log(
                `[src/buttons/server/owner/userFlags.ts] ${inspect(error, {depth: Infinity, colors: true, compact: false})}`,
              );
            });
        }
      }

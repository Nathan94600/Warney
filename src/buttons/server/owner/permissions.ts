...
import { createInteractionResponse, getPermissionLabels } from "../../../utils/functions/others";
	...
	run(client, interaction, authorId) {
		...
		if (!guildId) createInteractionResponse(interaction, { type: InteractionCallbackTypes.ChannelMessageWithSource, data: { content: "Server id not found", flags: [MessageFlags.Ephemeral] } });
			...
			if (!guildInCache) createInteractionResponse(interaction, { type: InteractionCallbackTypes.ChannelMessageWithSource, data: { content: "Server not found", flags: [MessageFlags.Ephemeral] } });
				...
				const ownerId = guildInCache.ownerId, owner = guildInCache.members.get(ownerId), ownerPermissions = owner?.permissions;
				...
				else if (!ownerPermissions) createInteractionResponse(interaction, { type: InteractionCallbackTypes.ChannelMessageWithSource, data: { content: "Owner permissions not found", flags: [MessageFlags.Ephemeral] } });
				else {
					const fields: EmbedField[] = [], permissionLabels = getPermissionLabels(ownerPermissions);

					for (let i = 0; i < permissionLabels.length; i += 2) {
						const permission = permissionLabels[i], nextPermission = permissionLabels[i + 1];

						if (permission) fields.push({ name: permission, value: nextPermission || "\u200b", inline: true });
					};

					createInteractionResponse(interaction, { type: InteractionCallbackTypes.UpdateMessage, data: {
						embeds: [{
							author: {
								name: `Permissions of ${owner.nick || owner.user.globalName} (${ownerId})`,
							...
							fields: fields.length == 0 ? [{ name: "No permissions", value: "\u200b" }] : fields
						}],
						components: [{
						...
						console.log(`[src/buttons/server/owner/permissions.ts] ${inspect(error, { depth: Infinity, colors: true, compact: false })}`);
					});
				}
			}
		};

    ...
    if (!guildId)
      ...
      if (!guildInCache)
        ...
        const ownerId = guildInCache.ownerId,
          ...
          ownerFlags: any[] = []; //owner?.user.publicFlags;

        if (!owner)
          interaction.reply({content: "Owner not found", flags: ["Ephemeral"]});
        else if (!ownerFlags)
          interaction.reply({
            content: "Owner public user flags not found",
            flags: ["Ephemeral"],
          });
        else {
          ...
          interaction
            .update({
              embeds: [
                {
                  author: {
                    name: `Public user flags of ${owner.displayName} (${ownerId})`,
                    icon_url: owner.displayAvatarURL(),
                  },
                  fields:
                    fields.length == 0
                      ? [{name: "No public user flags", value: "\u200b"}]
                      : fields,
                },
              ],
              components: [
            ...
            .catch((error) => {
              ...
              console.log(
                `[src/buttons/server/owner/publicFlags.ts] ${inspect(error, {depth: Infinity, colors: true, compact: false})}`,
              );
            });
        }
      }

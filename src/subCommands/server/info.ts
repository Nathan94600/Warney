...
export default {
	...
	run(client, interaction) {		
		const guildId = interaction.guild?.id || interaction.guild_id, guild = client.cache.guilds.get(guildId || "");
		...
		else createInteractionResponse(interaction, { type: InteractionCallbackTypes.ChannelMessageWithSource, data: {
			embeds: [{
				...
				fields: [
					...
					{ name: `Features (${guild.features.length})`, value: "(**/server features**)", inline: true },

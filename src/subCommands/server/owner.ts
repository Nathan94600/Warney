...
export default {
	...
	run(client, interaction) {		
		const guildId = interaction.guild?.id || interaction.guild_id, guild = client.cache.guilds.get(guildId || ""), owner = guild?.members.get(guild?.ownerId || ""), authorId = (interaction.member?.user || interaction.user)?.id;
		...
		if (!guildId) createInteractionResponse(interaction, { type: InteractionCallbackTypes.ChannelMessageWithSource, data: { content: "You must be in a server", flags: [MessageFlags.Ephemeral] } });
			...
			createInteractionResponse(interaction, { type: InteractionCallbackTypes.ChannelMessageWithSource, data: {
				embeds: [{
					author: { name: `${owner.nick || owner.user.globalName} (${ownerId})`, iconUrl: ownerIcon },
